...
updaterc() {
	if < /etc/os-release grep "ID_LIKE=.*alpine.*\|ID=.*alpine.*" ; then
		echo "Updating /etc/profile"
		echo -e "$1" >>/etc/profile
	fi

	for shellrc in "/etc/zsh/zshrc" "/usr/bin/fish"  "/etc/bash.bashrc" ; do
		if [ -f "${shellrc}" ] && [[ "$(cat $shellrc)" != *"$1"* ]]; then
			echo "Updating ${shellrc}"
			echo -e "$1" >> $shellrc
		fi
	done;
}
...
install_via_asdf() {
	...
	elif < /etc/os-release grep "ID_LIKE=.*debian.*\|ID=.*debian.*"; then
